...
const StockUse = () => {
  ...
  //獲取產業鏈資料
  const fetchIndustryChain = async () => {
    try {
      const res1 = await axios.get(`${API_BASE}/api/finmind/TaiwanStockIndustryChain`);
      // 實際呼叫: https://api.finmindtrade.com/api/v4/data?dataset=TaiwanStockIndustryChain
      console.log('所有產業鏈資料', res1.data); // 所有產業鏈資料
    } catch (error) {
      console.error('批次讀取錯誤:', error);
      setError('批次讀取失敗');
    } finally {
